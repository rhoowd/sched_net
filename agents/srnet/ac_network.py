...
class ActorNetwork:
    def __init__(self, sess, state_dim, action_dim, nn_id=None):
        ...
        self.conn = tf.placeholder(tf.bool, name='check')
        ...
        with tf.variable_scope(scope):
            ...
            self.actions = self.generate_actor_network(self.state_ph, self.conn, trainable=True)
    ...
    def action_for_state(self, state_ph):
        ...
        return self.sess.run(self.actions,
                             feed_dict={self.state_ph: state_ph, self.is_training_ph: False, self.conn: self.conn_i})
