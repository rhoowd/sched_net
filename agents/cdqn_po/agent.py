...
import math
from agents.cdqn_po.dq_network import *
from agents.cdqn_po.replay_buffer import *
...
class Agent(object):
    ...
    def __init__(self, action_dim, obs_dim, name=""):
        logger.info("Centralized DQN PO Agent")
    ...
    def act(self, state, obs):
        ...
        self.is_observed(obs)
        state_i = self.state_to_index(state, obs)
    ...
    def train(self, state, action, reward, state_n, done, obs, obs_n):

        a_i = self.action_to_index(action[0], action[1])
        a = self.onehot(a_i, self._action_dim)
        s = self.state_to_index(state, obs)
        s_n = self.state_to_index(state_n, obs_n)
        r = np.sum(reward)
    ...
    def state_to_index(self, state, obs):
        """
    ...
    def is_observed(self, obs):
        ret = False
        for i in range(2):
            if np.shape(np.where(obs[i] == 4))[1] > 0:
                ret = True
        return ret
